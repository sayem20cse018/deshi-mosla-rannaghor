...
import { useAdminBrand, useUpdateBrand } from '@/hooks/useAdminBrands';
...
export default function EditBrandPage() {
  ...
  const { data: brand, isLoading, error } = useAdminBrand(id);
  ...
  if (isLoading) return <LoadingState message="Loading brand..." />;
  if (error)     return <ErrorState message="Could not load brand" />;
    ...
    <form onSubmit={handleSubmit} className="space-y-5 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-black text-gray-900">Edit Brand</h1>
          <p className="text-sm text-gray-400 mt-0.5">{form.name || 'Editing brand'}</p>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.name} onChange={set('name')} placeholder="Brand name" className={inputCls} required />
              </div>
              <div>
