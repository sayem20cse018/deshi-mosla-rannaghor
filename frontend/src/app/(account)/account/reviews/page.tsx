...
const STATUS_LABEL: Record<string,string> = {APPROVED:'অনুমোদিত',PENDING:'অপেক্ষারত',REJECTED:'প্রত্যাখ্যাত'};

export default function ReviewsPage() {
  const { data, isLoading } = useQuery({
    queryKey: ['my-reviews'],
    queryFn: async () => { const r = await api.get('/users/me/reviews'); return r.data.data; },
  });
    ...
    <div>
      ...
      {!data?.length ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <Link href="/shop" className="btn-primary px-6 text-sm">পণ্য দেখুন</Link>
        </div>
      ) : (
          ...
          {data.map((review: any) => (
            <div key={review.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              <div className="flex items-start gap-4">
                {/* Product image */}
                <Link href={`/product/${review.product?.slug}`} className="w-14 h-14 rounded-xl overflow-hidden bg-brand-50 flex items-center justify-center flex-shrink-0 border border-gray-100 hover:border-brand-200 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <Link href={`/product/${review.product?.slug}`} className="font-semibold text-gray-800 text-sm hover:text-brand-700 transition-colors">
                      {review.product?.name}
                    </Link>
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border flex-shrink-0 ${STATUS_COLOR[review.status]}`}>
