...
const STATUS_LABEL: Record<string, string> = {
  ...
  PACKED:'প্যাক',SHIPPED:'পাঠানো',DELIVERED:'ডেলিভারি',
  CANCELLED:'বাতিল',RETURNED:'ফেরত',REFUNDED:'রিফান্ড',
};
const PAY_LABEL: Record<string,string> = {
  PENDING:'অপেক্ষারত',PAID:'পরিশোধিত',FAILED:'ব্যর্থ',
  CANCELLED:'বাতিল',REFUNDED:'ফেরত',PROCESSING:'প্রক্রিয়াধীন',
};
...
export default function OrderDetailPage() {
  ...
  if (error || !data) return (
    <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
      ...
      <Link href="/account/orders" className="btn-primary px-6 text-sm">অর্ডার তালিকায় ফিরুন</Link>
    </div>
  );
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      {/* Back */}
      <Link href="/account/orders" className="flex items-center gap-1.5 text-sm text-brand-700 hover:text-brand-800 font-medium">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-400 text-xs mt-1">
              {new Date(data.createdAt).toLocaleDateString('bn-BD', { year:'numeric',month:'long',day:'numeric',hour:'2-digit',minute:'2-digit' })}
            </p>
          </div>
          <div className="text-right">
            ...
            <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-full',
              data.paymentStatus === 'PAID' ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'
            )}>
              ...
              {STATUS_STEPS.map((s, i) => (
                <div key={s} className="flex flex-col items-center flex-1">
                  <div className={cn(
                    ...
                    i === currentStep ? 'bg-white border-brand-600 text-brand-600 ring-2 ring-brand-200' :
                    'bg-white border-gray-200 text-gray-300',
                  )}>
                    {i < currentStep ? <CheckCircle className="w-3.5 h-3.5" /> : i + 1}
                  </div>
                  <p className={cn('text-[10px] mt-1 text-center leading-tight',
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          {data.items?.map((item: any) => (
            <div key={item.id} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
              <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center text-xl flex-shrink-0">
                {item.productImage ? <img src={item.productImage} alt={item.productName} className="w-full h-full rounded-xl object-cover" /> : '🌶️'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800 text-sm leading-tight truncate">{item.productName}</p>
                <p className="text-gray-400 text-xs mt-0.5">{item.productSku} · {item.quantity} × {formatPriceEn(item.unitPrice)}</p>
              </div>
        ...
        <div className="mt-4 space-y-1.5 border-t border-gray-100 pt-4">
          <div className="flex justify-between text-sm text-gray-500"><span>সাবটোটাল</span><span>{formatPriceEn(data.subtotal)}</span></div>
          {data.discountAmount > 0 && <div className="flex justify-between text-sm text-green-600"><span>ছাড়</span><span>−{formatPriceEn(data.discountAmount)}</span></div>}
          {data.couponDiscount > 0 && <div className="flex justify-between text-sm text-brand-600"><span>কুপন ছাড়</span><span>−{formatPriceEn(data.couponDiscount)}</span></div>}
          <div className="flex justify-between text-sm text-gray-500"><span>ডেলিভারি</span><span>{data.deliveryCharge > 0 ? formatPriceEn(data.deliveryCharge) : 'ফ্রি'}</span></div>
          <div className="flex justify-between font-black text-base text-gray-900 pt-2 border-t border-gray-100 mt-2">
            <span>সর্বমোট</span><span className="text-brand-700">{formatPriceEn(data.totalAmount)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {data.address && (
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            ...
            <p className="text-gray-500 text-sm">{data.address.area}, {data.address.district}</p>
        ...
        {data.payment && (
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            ...
            <p className="text-gray-700 text-sm"><span className="font-semibold">পদ্ধতি:</span> {data.payment.paymentMethod?.replace(/_/g,' ')}</p>
            <p className="text-gray-700 text-sm mt-1"><span className="font-semibold">স্ট্যাটাস:</span>{' '}
              <span className={cn('font-bold', data.payment.paymentStatus === 'PAID' ? 'text-green-600' : 'text-yellow-600')}>
                {PAY_LABEL[data.payment.paymentStatus]}
              </span>
            </p>
            {data.payment.transactionId && (
              <p className="text-gray-500 text-xs mt-2 font-mono break-all">TxnID: {data.payment.transactionId}</p>
            )}
            {data.payment.paidAt && (
              <p className="text-gray-400 text-xs mt-1">{new Date(data.payment.paidAt).toLocaleDateString('bn-BD')}</p>
            )}
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            {[
              ...
              { label: 'প্রত্যাশিত', value: data.delivery.estimatedDate ? new Date(data.delivery.estimatedDate).toLocaleDateString('bn-BD') : null },
