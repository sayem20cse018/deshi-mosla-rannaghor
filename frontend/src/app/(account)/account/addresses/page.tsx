...
const EMPTY_ADDR = { label: '', fullName: '', phone: '', division: '', district: '', area: '', fullAddress: '', postalCode: '', isDefault: false };
...
export default function AddressesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => { const r = await api.get('/users/me/addresses'); return r.data.data; },
  });

  function openAdd()   { setEditing(null); setForm({ ...EMPTY_ADDR }); setErrors({}); setShowForm(true); }
  function openEdit(a: any) { setEditing(a); setForm({ ...EMPTY_ADDR, ...a }); setErrors({}); setShowForm(true); }
    ...
    <div>
      ...
      {data?.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
          ...
          <button onClick={openAdd} className="btn-primary text-sm">ঠিকানা যোগ করুন</button>
        </div>
      ) : (
        <div className="space-y-3">
          {data?.map((addr: any) => (
            <div key={addr.id} className={cn('bg-white rounded-2xl border p-5 shadow-sm', addr.isDefault ? 'border-brand-200' : 'border-gray-100')}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {addr.isDefault && <span className="text-xs bg-brand-50 text-brand-600 border border-brand-200 px-2 py-0.5 rounded-full font-semibold">ডিফল্ট</span>}
                    </div>
                    <p className="text-gray-600 text-sm mt-1">{addr.fullAddress}</p>
                    <p className="text-gray-500 text-sm">{addr.area}, {addr.district}, {addr.division}</p>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  {!addr.isDefault && (
                    <button onClick={() => handleSetDefault(addr.id)} className="btn-icon text-gray-400 hover:text-amber-500" title="ডিফল্ট করুন">
                      <Star className="w-4 h-4" />
                    </button>
                  )}
                  <button onClick={() => openEdit(addr)} className="btn-icon text-gray-400 hover:text-brand-600">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-white px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-900">{editing ? 'ঠিকানা সম্পাদনা' : 'নতুন ঠিকানা'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              {[
                ...
                { key:'fullAddress', label:'সম্পূর্ণ ঠিকানা *', ph:'বাড়ি/রোড/মহল্লা',req:true },
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">বিভাগ *</label>
                  <select value={form.division} onChange={(e) => setForm(f => ({ ...f, division: e.target.value }))} className={cn('input-base', errors.division && 'border-red-400')}>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">জেলা *</label>
                  <input type="text" value={form.district} onChange={(e) => setForm(f => ({ ...f, district: e.target.value }))} placeholder="ঢাকা" className={cn('input-base', errors.district && 'border-red-400')} />
                  {errors.district && <p className="text-red-500 text-xs mt-1">{errors.district}</p>}
                </div>
              </div>

              <label className="flex items-center gap-2.5 cursor-pointer">
                <input type="checkbox" checked={form.isDefault} onChange={(e) => setForm(f => ({ ...f, isDefault: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 text-brand-600" />
                <span className="text-sm text-gray-700 font-medium">ডিফল্ট ঠিকানা হিসেবে সেট করুন</span>
              </label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="button" onClick={() => setShowForm(false)} className="btn-secondary flex-1 justify-center">বাতিল</button>
              </div>
            </form>
          </div>
        </div>
