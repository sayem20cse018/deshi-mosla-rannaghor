...
const BADGES = [
  {
    ...
    sub: 'কৃত্রিম উপাদান ছাড়া',
    color: 'bg-green-50  text-green-700',
  },
  {
    icon: Shield,
    title: 'নিরাপদ পেমেন্ট',
    sub: 'SSL এনক্রিপ্টেড সুরক্ষা',
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {BADGES.map(({ icon: Icon, title, sub, color }) => (
            <div key={title} className="flex items-center gap-3 py-1">
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${color.split(' ')[0]}`}
              >
                <Icon className={`w-4.5 h-4.5 ${color.split(' ')[1]}`} />
              </div>
              <div className="min-w-0">
                <p className="text-gray-800 font-semibold text-xs leading-tight truncate">
                  {title}
                </p>
