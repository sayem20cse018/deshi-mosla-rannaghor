...
import { Clock, Zap, CalendarClock, CheckCircle } from 'lucide-react';
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="font-bold text-gray-900 text-sm leading-tight" style={{ fontFamily: 'Noto Sans Bengali, Manrope, sans-serif' }}>
                  {provider.name}
                </p>
                {/* Type badge */}
                <span
              ...
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                {/* Estimated time */}
                <span className="flex items-center gap-1 text-[11px] text-gray-500" style={{ fontFamily: 'Noto Sans Bengali, sans-serif' }}>
                  <Clock className="w-3 h-3" /> {timeLabel}
                </span>
                {/* Note */}
                {provider.note && (
                  <span className="text-[11px] text-gray-400" style={{ fontFamily: 'Noto Sans Bengali, sans-serif' }}>
                    · {provider.note}
                  </span>
                )}
              </div>
            ...
            <div className="text-right flex-shrink-0">
              ...
              >
                {provider.fee === 0 ? '?? ????' : formatPriceEn(provider.fee)}
              </p>
              {selected && (
